...
import { XToken, Staking } from "../typechain";
import { deployXToken, deployStaking } from "../utils/deployContracts"
...
describe("Test", function () {
  ...
  beforeEach(async ()=>{
    ...
    staking = await deployStaking(xToken.address);
  })

  it("Deposit works", async() => {
    let amount = "150000000000000000000"
    await xToken.connect(await getGnosisWithEther(xToken)).mint(await user.getAddress(), amount)
    await xToken.connect(user).approve(staking.address, amount);

    expect(await xToken.balanceOf(await user.getAddress())).to.be.equal(amount)
  ...
  it("Multiple earnings", async() => {
    ...
    await xToken.connect(await getGnosisWithEther(xToken)).mint(await user.getAddress(), amountSum)
    await xToken.connect(user).approve(staking.address, amountSum);
  ...
  it("Earning works correctly after 1 year", async() => {
    await xToken.connect(await getGnosisWithEther(xToken)).mint(staking.address, "1000000000000000000000000")
  ...
  it("Claim and Withdraw works", async() => {
    ...
    expect(await xToken.balanceOf(await user.getAddress())).not.to.be.equal("0")
  })
  ...
  it("add works", async() => {
    ...
    await xToken.connect(await getGnosisWithEther(xToken)).mint(staking.address, maxValue)
    let amount  =    "100000000000000000000"
    let amount10Percentage_half  =    "5000000000000000000"
    await xToken.connect(await getGnosisWithEther(xToken)).mint(await user.getAddress(), amount + "0")
    await xToken.connect(user).approve(staking.address, amount + "0");
