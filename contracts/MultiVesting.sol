...
contract MultiVesting is IVesting, Ownable {
    ...
    event SellerSet(address newSeller);
    event UserVested(address indexed beneficiary, uint256 amount);
    event VestingEmergency(uint256 amount);
    event BeneficiaryUpdate(
        address indexed oldBeneficiary,
        address indexed newBeneficiary
    );
    event EarlyWithdrawDisable(address owner);

    IERC20 public immutable token;
    uint256 public sumVesting;
    address public seller;
    address public constant GNOSIS = 0x0000000000000000000000000000000000000001;
    ...
    function vest(
        ...
        emit UserVested(_beneficiaryAddress, _amount);
    }
    ...
    function finishUpdateBeneficiary(address _oldBeneficiary) external {
        ...
        emit BeneficiaryUpdate(it.oldBeneficiary, it.newBeneficiary);
    }
    ...
    function disableEarlyWithdraw() external onlyOwner {
        ...
        emit EarlyWithdrawDisable(msg.sender);
    }
}
